...
4. Drop/recompile: `RoadTrip_Table.lsl` + `RoadTrip_Http.lsl` (root), `RoadTrip_Track.lsl` + `RoadTrip_Screens.lsl` (Track), `RoadTrip_HUD.lsl`.
5. Reset Track — owner chat should say all 50 screen textures found (from Screens).
